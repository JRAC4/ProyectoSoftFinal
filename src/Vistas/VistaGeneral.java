...
//    SQL consulta = new SQL();
    ...
    ArrayList<Categoria> lista_Categoria = null;
    CategoriaDAO controladorCategoria = new CategoriaDAO();

    ArrayList<Marca> lista_Marca = null;
...
//    Fac_cat3_umeidaImpl controladorFac_cat3_umedida = new Fac_cat3_umeidaImpl();
    ...
    ArrayList<Igv> lista_Igv = null;
    IgvDAO controladorFac_cat7_igv = new IgvDAO();
    ...
    public void opcionde_Muestra() {
        ...
        if (nombreTabla.equalsIgnoreCase("categoria")) {
            ...
            lista_Categoria = (ArrayList<Categoria>) controladorCategoria.listarCnt(txtDescripcion.getText(), 30, id_sede);
            modeloLista.removeAllElements();
            for (int i = 0; i < lista_Categoria.size(); i++) {
                modeloLista.addElement(lista_Categoria.get(i).getDescripcion());//nombre es el campo de la base de datos 
                ListaElementos.setModel(modeloLista);
            }

        } else if (nombreTabla.equalsIgnoreCase("marca")) {

            lista_Marca = (ArrayList<Marca>) controladorMarca.listarCnt(txtDescripcion.getText(), 30, id_sede);
            modeloLista.removeAllElements();
            for (int i = 0; i < lista_Marca.size(); i++) {
                modeloLista.addElement(lista_Marca.get(i).getDescripcion());//nombre es el campo de la base de datos 
                ListaElementos.setModel(modeloLista);
            }
...
//            }
            ...
            lista_Igv = (ArrayList<Igv>) controladorFac_cat7_igv.listarCnt(txtDescripcion.getText(), 30);
            modeloLista.removeAllElements();
            for (int i = 0; i < lista_Igv.size(); i++) {
                modeloLista.addElement(lista_Igv.get(i).getDescripcion());//nombre es el campo de la base de datos 
                ListaElementos.setModel(modeloLista);
            }
...
//                opcionde_Muestra();
                //2
                int id_categoria_registro = sql.obtenerEntero("Select max(id_categoria) from categoria where id_usuario='" + id_usuario + "' and id_sede='" + id_sede + "'");
...
//                opcionde_Muestra();
                //2
                int id_marca_registro = sql.obtenerEntero("Select max(id_marca) from marca where id_usuario='" + id_usuario + "' and id_sede='" + id_sede + "'");
