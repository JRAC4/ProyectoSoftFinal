...
import controlador.UsuarioSedeDAO;
...
public class Form_Usuario extends javax.swing.JDialog {
    ...
    SQL sql = new SQL();//llamamos al mentenimiento general del objeto
...
//        this.id_sedes=id_usuarios;
    ...
    public Form_Usuario(java.awt.Frame parent, boolean modal, int id_usuario, int ver, int id_sede) {
        ...
        btnNuevoPerfil.setVisible(false);
//emp.icono.general
...
//        validar.validarSoloNumeros(txtDescripcion);
       int id_perfilGen = controlador.obtenerId_perfil(1);
        if (id_usuario == 0) {
            ...
            String descripcion = controlador.getUsuario().getUsuario();
            String password = controlador.getUsuario().getPassword();
            int id_perfil = controlador.getUsuario().getId_perfil();
...
//            ////////////////////////////////////////////////////////////////////////////
    ...
    public void lista_Perfil(int id_perfil) { 
        
        lista_Perfil = (ArrayList<Perfil>) controladorPerfil.listar();
//        if (perfil.equalsIgnoreCase("DESARROLLADOR")) {
//            lista_Perfil = (ArrayList<Perfil>) controladorPerfil.listar(); //listarSinDes
//        } else {
//            lista_Perfil = (ArrayList<Perfil>) controladorPerfil.listar(); //listar
//        }
        cboPerfil.removeAll();
        for (int i = 0; i < lista_Perfil.size(); i++) {
            cboPerfil.addItem(lista_Perfil.get(i).getDescripcion());
        }
    }
    ...
    public void lista_Usuario(int idUsuario) {//LISTA TODAS EMPRESAS   
        lista_UsuarioSede = (ArrayList<UsuarioSede>) controladorUsuarioSede.listarporUsuario(idUsuario);
        for (int i = 0; i < lista_UsuarioSede.size(); i++) {
            Object datos[] = new Object[2];

            datos[0] = (lista_UsuarioSede.get(i).getId_sede()) + "";
            datos[1] = (controladorSedes.obtieneDescripcion_id(lista_UsuarioSede.get(i).getId_sede())) + "";
    ...
    public void guardar_detalle(int idusuario) {
        ...
        if (filas <= 0) {//si no elije ninguna fila
            ...
            for (int i = 0; i <= (totalRow); i++) {
                ...
                try {
                    UsuarioSede usuSede = new UsuarioSede(0, idusuario, idSede);
    ...
    private void btnRegistrarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRegistrarActionPerformed
        ...
        try {
            ...
            if (id_usuario == 0) {
                ...
                Usuario usuario = new Usuario(0, leeDescripcion(), leePaassword(), 1,
                        leeIdPerfil(), id_sede);
                controlador.registrar(usuario);

                int idmax_usuario = sql.obtenerEntero("Select max(id_usuario) from usuario ");
                ...
                Usuario usuario = new Usuario(leeId(), leeDescripcion(), leePaassword(), 1,
                        leeIdPerfil(), id_sede);
...
private void Limpiar_Tabla() {
    ...
    private void btnNuevoPerfilActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnNuevoPerfilActionPerformed
        // new Form_Perfil(null, true, 0, 0).setVisible(true);
        //int id_perfil = controlador.getUsuario().getId_perfil();
        //        lista_Perfil(id_perfil);
        //
        //            int id_perfilRegistro = controladorPerfil.obtenerEntero("Select max(id_perfil) from perfil where estado=1");
        //            String Descripcion=controladorPerfil.obtieneDescripcion_id(id_perfilRegistro);
        //   cboPerfil.setSelectedItem(Descripcion);
